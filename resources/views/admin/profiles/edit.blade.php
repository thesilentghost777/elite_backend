...
@section('breadcrumb')
    ...
    <span>/</span>
    <span>Modifier {{ $profile->nom }}</span>
@endsection
...
<div class="form-container">
    <div class="page-header">
        <h1 class="page-title">Modifier le profil métier</h1>
        <p class="page-description">{{ $profile->nom }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.profiles.update', $profile) }}" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <div class="secteur-buttons">
                        ...
                        <label class="secteur-btn {{ old('secteur', $profile->secteur) == $secteur ? 'active' : '' }}">
                            <input type="radio" name="secteur" value="{{ $secteur }}" 
                                   {{ old('secteur', $profile->secteur) == $secteur ? 'checked' : '' }} required>
                            {{ $secteur }}
                        </label>
                        @endforeach
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <div class="debouches-list" id="debouchesList">
                        @if(old('debouches', $profile->debouches))
                            @foreach(old('debouches', $profile->debouches) as $debouche)
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="niveau_minimum" class="form-control @error('niveau_minimum') error @enderror" required>
                            ...
                            <option value="BEPC" {{ old('niveau_minimum', $profile->niveau_minimum) == 'BEPC' ? 'selected' : '' }}>BEPC</option>
                            <option value="Probatoire" {{ old('niveau_minimum', $profile->niveau_minimum) == 'Probatoire' ? 'selected' : '' }}>Probatoire</option>
                            <option value="BAC" {{ old('niveau_minimum', $profile->niveau_minimum) == 'BAC' ? 'selected' : '' }}>BAC</option>
                            <option value="Licence" {{ old('niveau_minimum', $profile->niveau_minimum) == 'Licence' ? 'selected' : '' }}>Licence</option>
                            <option value="Master" {{ old('niveau_minimum', $profile->niveau_minimum) == 'Master' ? 'selected' : '' }}>Master</option>
                        </select>
                ...
                <div style="display: grid; gap: 1rem; margin-top: 1rem;">
                    <div class="switch-container">
                        <label class="switch">
                            <input type="checkbox" name="is_cfpam" value="1" {{ old('is_cfpam', $profile->is_cfpam) ? 'checked' : '' }}>
                    ...
                    <div class="switch-container">
                        <label class="switch">
                            <input type="checkbox" name="active" value="1" {{ old('active', $profile->active) ? 'checked' : '' }}>
                            <span class="slider"></span>
                        </label>
                        <div class="switch-label">
            ...
            <div class="form-actions">
                <a href="{{ route('admin.profiles.show', $profile) }}" class="btn btn-secondary">
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                    Annuler
    ...
    <div class="danger-zone">
        ...
        <p class="danger-zone-description">
            La suppression de ce profil est irréversible. Tous les utilisateurs ayant choisi ce profil, 
            les roadmaps et les liens avec les packs seront définitivement perdus.
        </p>

        <form method="POST" action="{{ route('admin.profiles.destroy', $profile) }}" 
              onsubmit="return confirm('Êtes-vous absolument sûr de vouloir supprimer ce profil ? Cette action est IRRÉVERSIBLE et affectera {{ $profile->userChoices->count() }} utilisateur(s).');">
            @csrf
            @method('DELETE')
